{ inputs, pkgs, config, ... }:
{
  ...
  xdg.desktopEntries.vesktop-wayland = {
    name = "Vesktop (Wayland Safe)";
    exec = "${pkgs.vesktop}/bin/vesktop --user-data-dir=${config.xdg.dataHome}/vesktop --enable-features=UseOzonePlatform,WebRTCPipeWireCapturer --ozone-platform=wayland --disable-features=VaapiVideoDecode,VaapiVideoDecoder,UseChromeOSDirectVideoDecoder";
    icon = "vesktop";
